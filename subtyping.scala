...
object subtyping:
  ...
  def observeCounter(f: Counter => Unit): Int ={
    ...
    class C extends Counter {
    	...
    	override def decrement () = { count = count + 1 } // increment for num of times called
    	def getCount () : Int = count
    }
      val a = new C()
      f(a) // call f
      a.getCount() // return total num of increment/decrement calls
  }
  ...
  def observeCounterList(f: List[Counter] => Unit): List[Int] =
    class C extends Counter {
      ...
      // elements 1, 2, 3
       ...
       f(List(a, b, c));
      // results for 1, 2, 3
        lst = List(a.getCount(), b.getCount(), c.getCount())
      // return list of ints
        lst
  ...
  def observeCounterArray(f: Array[Counter] => Unit): Array[Int] =
    ...
    // make new array of length 3
      ...
      f(Array(a, b, c));
        arr = Array(a.getCount(), b.getCount(), c.getCount())
        arr
