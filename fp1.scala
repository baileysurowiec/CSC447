...
object fp1:
  ...
  // adds value of accumulator to the remaining list
  ...
  def max(xs: List[Int]): Int =
    ...
    val first = xs.head // first element in list 
    val next = xs.tail // tail returns a list of all elements except first 
    
    if next.isEmpty // list has only one element
      then first  // will return first (the single element ^)

    else
      val m = max(next)  // recursiely call max on remainder of list
      
      if first > m // compare m to first
        then
         first
      else
        m  
  ...
  // loops.
  def maxTailAux(xs: List[Int], accumulator: Int): Int = xs match{
    case Nil => accumulator // no elements in list, returm accumulator

    case _ => val m = maxTailAux(xs.tail, accumulator)

    if m > xs.head // check which is bigger
      then m
    else
      xs.head
  }
