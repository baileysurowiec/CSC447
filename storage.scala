...
object storage:
  ...
  val getAndSet: Int => (() => Int, Int => Unit) =
    (n: Int) => {
      var p = n; 
      var get: () => Int = () => p // reader
      var set:Int => Unit = (a:Int) => p = a // writer
      (get,set)
    }
  ...
  val getAndSetSpy: () => (() => Int, Int => (() => Int, Int => Unit)) = {
    var c = 0
    () => {
      val gS: Int => (() => Int, Int => Unit) ={
        (n: Int) => {
          var p = n; 
          var get: () => Int = () => p // reader
          var set:Int => Unit = (a:Int) => {
            p = a // writer
            c = c+1 // increase count with set 
          }
          (get,set)
        } 
      }
      var getC: () => Int = () => c // counter
      var getGs: Int => (() => Int, Int => Unit) = gS;
      
      (getC, getGs) // pair to return
    }
  }
